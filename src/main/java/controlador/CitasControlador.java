...
public class CitasControlador implements ActionListener {
    ...
    public CitasControlador(citasCrear vista, PersonaJson modeloPersona) {
        ...
        this.modeloCita = new CitasJson(modeloPersona); // Inicializa CitasJson con la instancia de PersonaJson
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == vistaCita.getButtonCrear()) {
            ...
            if (documentoPaciente.isEmpty() || documentoOdontologo.isEmpty() || fechaCita == null || horaCitaTexto.isEmpty() || descripcionCita.isEmpty()) {
                JOptionPane.showMessageDialog(vistaCita, "Todos los campos son obligatorios.", "Error", JOptionPane.ERROR_MESSAGE);
                return;
            }
            ...
            try {
                ...
                JOptionPane.showMessageDialog(vistaCita, "Formato de hora incorrecto (HH:mm).", "Error", JOptionPane.ERROR_MESSAGE);
                return;
            }

            // Combina la fecha y la hora en un solo String para el objeto Citas
            String fechaHoraCita = fechaCita.toString() + " " + horaFormateada.toString();

            Citas nuevaCita = new Citas(documentoPaciente, documentoOdontologo, fechaHoraCita, descripcionCita);
